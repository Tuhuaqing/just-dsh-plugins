// dsh-auto-update · Client 半边
// 本文件是 DSH 动态插件的 code.client（一个返回 Cordis Plugin 的函数体）。
// 职责：把版本号 + 更新/重启按钮渲染进「设置」按钮内部（settings.trigger），紧挨“设置”文字右侧。
...
return {
  ...
  apply(ctx) {
    ...
    const POLL_INTERVAL_MS = 10000 // 每 10 秒向 Host 拉取一次状态
    ...
    // 渲染进“设置”按钮内部：齿轮图标 + 本地化标签 + 版本号 + 更新/重启按钮
    function TriggerContent(props) {
      const wide = props.wide
      const t = props.t
      const [status, setStatus] = React.useState({ current: null, latest: null, updateAvailable: false, phase: 'checking', error: null })
      const [busy, setBusy] = React.useState(false)

      const refresh = async () => {
        try { setStatus(await host.call('getStatus')) } catch (_) { /* 忽略单次 RPC 失败 */ }
      }
      ...
      const onUpdate = async (e) => {
        ...
        setBusy(true)
        try { await host.call('installUpdate') } catch (_) { /* 由 refresh 拉取真实状态 */ }
        await refresh()
        setBusy(false)
      }
      ...
      const onRestart = async (e) => {
        if (e && e.stopPropagation) { e.stopPropagation(); e.preventDefault(); }
        setBusy(true)
        try { await host.call('restart') } catch (_) { /* 重启后页面将断开 */ }
        setBusy(false)
      }
      ...
      const installing = phase === 'installing' || busy
      const installed = phase === 'installed'
      const showUpdate = phase === 'idle' && status.updateAvailable && !busy

      let button = null
      if (installing) {
        button = React.createElement('button', { type: 'button', className: 'dsh-au-btn', disabled: true, 'aria-label': '安装中' }, React.createElement('span', { className: 'dsh-au-spinner' }))
      } else if (installed) {
      ...
      const label = typeof t === 'function' ? t('trigger') : '设置'
      const tip = status.error ? '检查失败: ' + status.error : (status.latest ? '最新版: ' + status.latest : '')

      return React.createElement(React.Fragment, null,
        SettingsIcon(16),
        React.createElement('span', { className: 'dsh-au-label' }, label),
        React.createElement('span', { className: 'dsh-au-right', title: tip },
